...
const { disable_fetching } = require('./config.json');
...
(async () => {
	...
	UrlsConfig.countDocuments({}, async (err, total) => {
		...
		if (disable_fetching == false) fetchProjects(client.projects, client);
	});
})();
...
//🚀 Backup-Panel WebSite
require("./webSite/server/server")(client); // Website Code Intilization

client.login(process.env.token || config.token).catch(e => { console.log("Can't login to bot ")})
